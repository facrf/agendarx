...
async fn uploads_persistencia_transacoes_e_permissoes() {
    ...
    .await;
    let relationship_path = format!("/api/vinculos/{relationship}");
    api.json(
        Method::DELETE,
        &relationship_path,
        user,
    ...
    api.json(
        Method::GET,
        relationship_attachment["url_stream"].as_str().unwrap(),
        user,
        Value::Null,
        StatusCode::NOT_FOUND,
    )
    ...
    assert_eq!(trash[0]["id"], relationship);
    api.json(
        Method::POST,
        "/api/vinculos",
        user,
        json!({"pessoa_origem_id": id, "pessoa_destino_id": second_person, "tipo_vinculo": "Teste", "descricao": null}),
        StatusCode::CONFLICT,
    )
    .await;
    api.json(
        Method::DELETE,
        &format!("/api/pessoas/{second_person}"),
        user,
        Value::Null,
        StatusCode::NO_CONTENT,
    )
    ...
    api.json(
        Method::POST,
        &format!("/api/vinculos/lixeira/{relationship}/restaurar"),
        user,
        Value::Null,
        StatusCode::CONFLICT,
    )
    ...
    api.json(
        Method::POST,
        &format!("/api/produtividade/lixeira/{second_person}/restaurar"),
        user,
        Value::Null,
        StatusCode::NO_CONTENT,
    )
    ...
    assert_eq!(
        api.json(
            Method::GET,
            &format!("{relationship_path}/anexos"),
            user,
            Value::Null,
            StatusCode::OK
        )
    ...
    api.json(
        Method::DELETE,
        &format!("/api/vinculos/lixeira/{relationship}"),
        admin,
        Value::Null,
        StatusCode::NOT_FOUND,
    )
    ...
    assert_eq!(
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM anexo_vinculo WHERE id = ?")
            .bind(relationship_attachment["id"].as_i64().unwrap())
            .fetch_one(&pool)
            .await
            .unwrap(),
        0
