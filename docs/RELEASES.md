...
git tag -a v0.3.0 -m "AgendarX v0.3.0"
...
pacotes e arquivos `.sha256`. Também publica no GHCR as tags `0.3.0`, `0.3`, `0`
e `latest`.
...
docker run --rm -p 12000:12000 ghcr.io/facrf/agendarx:0.3.0
```
...
```bash
sha256sum -c agendarx-0.3.0-linux-riscv64.tar.gz.sha256
tar -xzf agendarx-0.3.0-linux-riscv64.tar.gz
cd agendarx-0.3.0-linux-riscv64
cp .env.example .env
./agendarx
```
